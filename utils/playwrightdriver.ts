...
export class PlaywrightDriver {
    ...
    async clickElement(locator: Locator, timeout = 20000): Promise<void> {
        try {
            // await this.waitForCondition(async () => await locator.isVisible(), 5, 2000);
            await this.waitForCondition(async () => await locator.isEnabled(), 5, 2000);
    ...
    async isElementVisible(element: Locator) {
            ...
            if(await this.waitForCondition(async () => await element.isVisible(), 10, 1000)){
                console.log(`${element} is visible on the page`);
                return true;
            }
            else{
